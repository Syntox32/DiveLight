...
Menu::Menu(Config& config)
    : m_config(config)
{
    addBooleanAction(config.applySqrtPostFFT, Keyboard::Key::G);
    addFloatAction(config.linearScalingConstant, 0.5f, Keyboard::Key::Add, Keyboard::Key::Dash);

    actionScaling = new EnumAction<ScalingMethod>(config.scalingMethod, 0, 4, sf::Keyboard::Key::W, sf::Keyboard::Key::S, false);
    actionFreq = new EnumAction<FreqLimitMethod>(config.freqLimitMethod, 0, 2, sf::Keyboard::Key::E, sf::Keyboard::Key::D, false);
